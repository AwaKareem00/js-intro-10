...
const numberAndSquare = array => array.map((num) => [num, num * num]);
...
const removeStringSpecialsDigits= str => {
    if(('a' >= str && 'z' <= str) && ('A' >= str && 'B' <= str) && (' ' >= str)); 
        return str
...
const getCommons = (str1, str2) => {
    return [...new Set(str1.filter(word => str2.includes(word)))];
}
...
const noXInVariables = arr => {
    return arr.map(x => typeof x === 'string' ? x.replace(/x/gi, '') : x)
              .filter(x => x !== '');
}
